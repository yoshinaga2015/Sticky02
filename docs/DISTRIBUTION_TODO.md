...
**自動インストールウィンドウ付きDMGの作成（推奨）**

DMGを開いた時に、アプリケーションフォルダへの移動を促すウィンドウを自動的に表示するDMGを作成できます。

1. **スクリプトを使用してDMGを作成**
   ```bash
   # プロジェクトのルートディレクトリで実行
   ./scripts/create-dmg.sh /path/to/Sticky.app
   
   # 例: ビルドされたアプリを使用する場合
   ./scripts/create-dmg.sh ~/Library/Developer/Xcode/DerivedData/Sticky-*/Build/Products/Release/Sticky.app
   ```

2. **作成されるDMGの特徴**
   - DMGを開くと、自動的にウィンドウが表示されます
   - 左側に `Sticky.app`、右側に `Applications` フォルダ（エイリアス）が表示されます
   - ユーザーは `Sticky.app` を `Applications` フォルダにドラッグ&ドロップするだけでインストールできます
   - ウィンドウサイズとアイコンの位置が最適化されています

3. **作成されたDMGの確認**
   - `Sticky.dmg` ファイルが作成されます
   - ダブルクリックして開き、動作を確認してください

**手動でDMGを作成する場合（上級者向け）**

ディスクユーティリティを使用して手動でDMGを作成することもできます：
...
5. マウントされたDMGに `Sticky.app` をドラッグ&ドロップ
6. アプリケーションフォルダ（`/Applications`）へのエイリアス（ショートカット）を作成して追加
   - ターミナルで以下を実行:
     ```bash
     ln -s /Applications "/Volumes/Sticky/Applications"
     ```
   
7. DMGをアンマウント（保存）
   - マウントされたDMGのウィンドウを閉じる
   - Finderのサイドバーで「Sticky」を右クリック → 「取り出す」を選択
